...
rs2.open "select top 1000 * from fuhe_in order by id desc",conn,1,1
if not rs2.eof then   
do while not rs2.eof
%>
po_detail_show[<%=i%>][<%=j%>] = '<%=rs2("cname")%>|<%=rs2("guige")%>';
po_detail_value[<%=i%>][<%=j%>] = '<%=rs2("id")%>|<%=rs2("cname")%>|<%=rs2("guige")%>';
<%
rs2.movenext
j=j+1
loop
...
sub SaveAdd
  ...
  response.write "<meta http-equiv=""refresh"" content=""0;url=fuhe_in.asp"">"
  response.end
end sub
...
sub delCate()
  ...
  response.end
        ...
        conn.execute("delete from fuhe_in where id in ("&Request.Form("selBigClass")&")")
  ...
  <TR> 
	...
	<%
	   ...
	   set rs=server.createobject("adodb.recordset")
		   rs.open "select top 1000 * from fuhe_in where id=" & request("id"),conn,1,1
%>
...
<%end if %>
        <table width="800" border="0" cellpadding="0" cellspacing="0" bordercolor="#0055E6">
          <tr class="but"> 
			<td width="25%" height="18" align="center" class="but" onMouseDown="this.className='tddown'" onMouseUp="this.className='but'" onMouseOut="this.className='but'" colspan="2">出库日期</td>
			...
			品 名 | 规&nbsp; 格 </td>
			<td width="15%" height="18" align="center" class="but" onMouseDown="this.className='tddown'" onMouseUp="this.className='but'" onMouseOut="this.className='but'">领用车间</td>
			
          </tr>
		  <form name="powersearch" method="post" action="fuhe_in.asp" onSubmit="return Juge(this)" >
...
<td height="18" align="center" class="but11" onMouseDown="this.className='tddown'" onMouseUp="this.className='but'" onMouseOut="this.className='but11'">
			...
			<% if isedit then%>
			<option selected="selected" value="0|<%=rs("cname")%>|<%=rs("guige")%>"><%=rs("cname")%>|<%=rs("guige")%></option>
...
<td height="18" align="center" class="but11" onMouseDown="this.className='tddown'" onMouseUp="this.className='but'" onMouseOut="this.className='but11'">
<select name="use_dep"  size="1" style='width:89; height:19'>
...
&nbsp;&nbsp;&nbsp;&nbsp; 2、单位为千米。</td>
		  ...
		  <tr> 
 <td height="18" align="center" class="but11" onMouseDown="this.className='tddown'" onMouseUp="this.className='but'" onMouseOut="this.className='but11'">
 <input name="one_fu" type="text"  size="7" value='<% if isedit then
...
<b><font color="#ffffff">复 合 出 库 列 表</font></b>
			...
			<form action="fuhe_in.asp" method="post" name="selform" >
...
rs.Source="select top 1000 * from fuhe_in where cname like '%"&keyword&"%'and  use_dep<>'null' order by -id"
else
rs.Source="select top 1000 * from fuhe_in where use_dep<>'null' order by -id"
end if
...
<td height="18" align="center"><a href="fuhe_in.Asp?id=<%=rs("ID")%>&action=edit"><%=rs("cname")%></a></td>
...
<b><font color="#ffffff">入库搜索</font></b>
        ...
        <table width="90%" border="0" cellpadding="0" cellspacing="0" bordercolor="#D4D0C8">
		  ...
		  <form name="search" method="post" action="fuhe_in.asp">
