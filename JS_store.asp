...
kehu=trim(request.Form("kehu"))
  ...
  response.write "<meta http-equiv=""refresh"" content=""0;url=JS_store.asp"">"
  response.end
else
        conn.execute("delete from JS_huzong where id in ("&Request.Form("selBigClass")&")")
...
<td >
			...
			<form action="JS_store.asp" method="post" name="selform" >
...
<font color="#FFFFFF"><b>&nbsp;客 户 </b></font><b><font color="#ffffff">长 欠 列 表</font></b>
...
rs.Source="select* from JS_huzong where kehu like '%"&keyword&"%' order by -id"
else
rs.Source="select* from JS_huzong  order by -id"
end if
...
<td height="18" align="center"><%=rs("kehu")%></td>
<td height="18" align="center"><%=Formatnumber(rs("Loan_Amount"),2,-1,0,0)%></td>
<td height="18" align="center"><%=Formatnumber(rs("number"),2,-1,0,0)%></td>
<td height="18" align="center"><%if rs("Loan_Amount")-rs("number")>0 then%>欠&nbsp;&nbsp;<b><font color="#FF0000"><%=Formatnumber((rs("Loan_Amount")-rs("number")),2,-1,0,0)%></font></b><%else%>长&nbsp;&nbsp;<%=Formatnumber((rs("number")-rs("Loan_Amount")),2,-1,0,0)%><%end if%></td>
...
<p align="right">共 <%=total%> 条，当前第 <%=Mypage%>/<%=Maxpages%> 
            ...
            <%
			if search<>"" then
			url="JS_store.asp?keyword="&keyword&"&search=search&"
			else
            url="JS_store.asp?"
			end if				
...
<td>
          ...
          <tr >  
<td height=26 background="images/background.gif">
<b><font color="#ffffff">&nbsp;客 户 长 欠 搜 索</font></b>
        ...
        <table width="100%" border="0" cellpadding="0" cellspacing="0" bordercolor="#D4D0C8">
		  ...
		  <form name="search" method="post" action="JS_store.asp">
		  <tr><input name="search" type="hidden" value="search">
		   ...
		   rs2.open "select distinct(kehu) from JS_huzong order by kehu",conn,1,1
		   if not rs2.eof then
