...
<table width="800" border="0" cellpadding="0" cellspacing="0" bordercolor="#D4D0C8">
	<form name="powersearch" method="post" action="cailiao_out_Compiles.asp?action=search" >
  ...
  <select name='pinming'  style='width:130'>
		   ...
		   rs2.open "select distinct(pinming) from cailiao_out_store order by pinming",conn,2,2
		   if not rs2.eof then
...
<tr>
  ...
  <td><input type="checkbox" name=SeachModel   value=4>按领用车间</td>
...
do while not rs.eof
%>
<option value="<%=rs("chejianname")%>"><%=rs("chejianname")%></option>
<%
rs.movenext
loop
rs.close
...
<td><select name="guige"  style='width:130'>
		   ...
		   rs3.open "select distinct(guige) from cailiao_out_store order by guige",conn,2,2
		   if not rs3.eof then
...
<table id = "PrintA" width="800" border="1" cellpadding="0" cellspacing="1" bordercolor="#EFEADD">
          <tr class="but"> 
                        <td width="100" height="18" align="center" class="but" onMouseDown="this.className='tddown'" onMouseUp="this.className='but'" onMouseOut="this.className='but'">日期</td>
                        <td width="61" align="center" class="but" onMouseDown="this.className='tddown'" onMouseUp="this.className='but'" onMouseOut="this.className='but'">类 别</td>
                        <td width="91" align="center" class="but" onMouseDown="this.className='tddown'" onMouseUp="this.className='but'" onMouseOut="this.className='but'">品 名</td>
                        <td width="149" align="center" class="but" onMouseDown="this.className='tddown'" onMouseUp="this.className='but'" onMouseOut="this.className='but'">
						产品名</td>
                        <td width="70" align="center" class="but" onMouseDown="this.className='tddown'" onMouseUp="this.className='but'" onMouseOut="this.className='but'">规 格</td>
                        <td width="133" align="center" class="but" onMouseDown="this.className='tddown'" onMouseUp="this.className='but'" onMouseOut="this.className='but'">数 量</td>
			<td width="55" align="center" class="but" onMouseDown="this.className='tddown'" onMouseUp="this.className='but'" onMouseOut="this.className='but'">单 位</td>
			<td width="120" align="center" class="but" onMouseDown="this.className='tddown'" onMouseUp="this.className='but'" onMouseOut="this.className='but'">领用车间</td>
...
<td height="18" align="center" ><a href="cailiao_out_Store.Asp?id=<%=rs("ID")%>&action=edit"><%=rs("uptime")%></a></td>

<td align="center"><font color="#CC3300"><%=rs("fenlei")%></font><%if rs("fenlei")="" then%>&nbsp;<%end if%></td>
<td align="center">&nbsp;<a href="cailiao_out_Store.Asp?id=<%=rs("ID")%>&action=edit"><font color="#000099"><%=rs("pinming")%></font></a></td>
<td align="center"><%=rs("cname")%></font><%if rs("cname")="" then%>&nbsp;<%end if%></td>
...
<td align="center"><font color="#666666"><%if rs("use_dep")<>"" then%><%=rs("use_dep")%><%else%>调为<%=rs("kufang")%><%=rs("old_fenlei")%><%end if%></td>
...
%>
                   <tr><td colspan="4" height="30" bgcolor="#FFFFFF">  符合查询条件的出库记录总共为 <font color="#cc0000"><%=i%></font> 条</td>
					...
					<td align="center" bgcolor="#FFFFFF"><font color="#cc0000"><%=Loan_num%></font></td>
</tr>
		
        </table>
		<br>
				<table cellpadding="0">
				<form name="search" method="post" action="cailiao_out_Detail.asp" target="_blank" >
				  <tr><td>
