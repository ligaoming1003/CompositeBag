...
{
	if (powersearch.houdou.value == "")
	{
		alert("规格不能为空！");
		powersearch.houdou.focus();
		return (false);
	}
}
...
sql="select * from houdou where houdou="& request.Form("houdou")&""
rs.open sql,conn,1,3
if not rs.eof then
response.Write "<script language=javascript>alert('对不起，你现在添加的规格已经存在！');</script>"
response.write "<meta http-equiv=""refresh"" content=""0;url=houdou.asp"">"
response.end
else
rs.addnew
rs("houdou") = request.Form("houdou")
...
sub SaveModify   
set rs=server.createobject("adodb.recordset") 
sql="select * from houdou where id="&request.Form("id")
rs.open sql,conn,1,3
rs("houdou") = request.Form("houdou")
...
response.end
  ...
  sub delCate()
        conn.execute("delete from houdou where id in ("&Request.Form("selBigClass")&")")
...
<TABLE width=600 align=center border="1" cellspacing="0" cellpadding="0" bordercolor="#0055E6">
  ...
  <TR> 
	...
	<%
	    set rs=server.createobject("adodb.recordset")
	   if isedit then
		   rs.open "select * from houdou where id=" & request("id"),conn,1,1
%>

    <table width="600" border="0" cellpadding="0" cellspacing="0" >
          <tr >  
<td height=26 background="images/background.gif">
<b><font color="#ffffff">编 辑 厚 度 信 息</font></b>
...
%>
    <table width="600" border="0" cellpadding="0" cellspacing="0" >
          <tr >  
<td height=26 background="images/background.gif">
<b><font color="#ffffff">添 加 厚 度 信 息</font></b>
...
<%end if %>
       <table width="600" border="0" cellpadding="0" cellspacing="0" bordercolor="#0055E6">
          <tr class="but"> 
            ...
            <td width="600" height="18" align="center" class="but" onMouseDown="this.className='tddown'" onMouseUp="this.className='but'" onMouseOut="this.className='but'">
			规格名称</td>


			
          </tr>
		  <form name="powersearch" method="post" action="houdou.asp" onSubmit="return Juge(this)" >
...
<input name="houdou" type="text" style="ime-mode:disabled"  size="25" value='<% if isedit then
...
<td >
			...
			<form action="houdou.asp" method="post" name="selform" >
  <table width="600"  border="1" cellpadding="0" cellspacing="0" bordercolor="#0055E6" align="center">
    ...
    <tr> 
      ...
      <td width="600" align="center" valign="top">
          ...
          <tr >  
<td height=26 background="images/background.gif">
<b><font color="#ffffff">&nbsp;厚 度 明 细 列 表</font></b>
        ...
        <table width="100%" border="1" cellpadding="0" cellspacing="1" bordercolor="#EFEADD">
          <tr class="but"> 
                        ...
                        <td width="500" height="18" align="center" class="but01" onMouseDown="this.className='tddown'" onMouseUp="this.className='but'" onMouseOut="this.className='but01'">
						名称</td>
...
if search<>"" then
fenlei1=request("houdou")
keyword=request("keyword")
rs.Source="select* from houdou where houdou like '%"&fenlei1&"%' order by id desc"
else
rs.Source="select* from houdou order by id desc"
end if
...
<td height="18" align="center">&nbsp;&nbsp;<a href="houdou.asp?id=<%=rs("ID")%>&action=edit"><%=rs("houdou")%></a></td>
...
共 <%=total%> 条，当前第 <%=Mypage%>/<%=Maxpages%> 
            ...
            <%
			if search<>"" then
			url="houdou.asp?houdou="&fenlei1&"&keyword="&keyword&"&search=search&"
			else
            url="houdou.asp?"
			end if				
...
<td>
          ...
          <tr >  
<td height=26 background="images/background.gif">
<b><font color="#ffffff">&nbsp;厚 度 信 息 搜 索</font></b>
        ...
        <table width="600" border="0" cellpadding="0" cellspacing="0" bordercolor="#D4D0C8">
		  ...
		  <form name="search" method="post" action="houdou.asp">
		  <tr><input name="search" type="hidden" value="search">
            <td align="center" width="20%" height="40" valign="middle">
			<select name="houdou" size="1">
			<option value="">所有规格</option>
			<%
	       set rs2=server.createobject("adodb.recordset")
		   rs2.open "select * from houdou",conn,1,1
		   if not rs2.eof then
		   do while not rs2.eof
		  houdou=rs2("houdou")
		   %>
		   <option value="<%=rs2("houdou")%>"><%=rs2("houdou")%></option>
			<%
			rs2.movenext
			loop
			end if
