...
<%
  ...
  response.write "<meta http-equiv=""refresh"" content=""0;url=chengping_store.asp"">"
  response.end
else
...
<b><font color="#ffffff">库 存 成 品 列 表</font></b>
			...
			<form action="chengping_store.asp" method="post" name="selform" >
...
rs.Source="select* from chengping_Store where number<>0 and fenlei like '%"&fenlei1&"%' and pinming like '%"&keyword&"%' order by -id"
else
rs.Source="select* from chengping_Store where number<>0 order by -id"
end if
...
<td height="18" align="center"><%=Formatnumber(rs("number"),2,-1,0,0)%><%if rs("number")="0" then%>&nbsp;<%end if%></td>
...
共 <%=total%> 条，当前第 <%=Mypage%>/<%=Maxpages%> 
            ...
            <%
			if search<>"" then
			url="chengping_Store.asp?fenlei="&fenlei1&"&keyword="&keyword&"&search=search&"
			else
            url="chengping_Store.asp?"
			end if				
...
<b><font color="#ffffff">库存搜索</font></b>
        ...
        <table width="800" border="0" cellpadding="0" cellspacing="0" bordercolor="#D4D0C8">
		  ...
		  <form name="search" method="post" action="chengping_store.asp">
