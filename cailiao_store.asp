...
sub SaveModify   
  ...
  response.write "<meta http-equiv=""refresh"" content=""0;url=cailiao_store.asp"">"
  response.end
else
        conn.execute("delete from cailiao_store where id in ("&Request.Form("selBigClass")&")")
...
<b><font color="#ffffff">库 存 材 料 列 表</font></b>
			...
			<form action="cailiao_store.asp" method="post" name="selform" >
      ...
      <td width="100%" align="center" valign="top">
        ...
        <table width="100%" border="1" cellpadding="0" cellspacing="1" bordercolor="#EFEADD">
          <tr class="but"> 
			...
			<td width="15%" height="18" align="center" class="but01" onMouseDown="this.className='tddown'" onMouseUp="this.className='but'" onMouseOut="this.className='but01'">单位</td>
			<td width="10%" height="18" align="center"  class="but01" onMouseDown="this.className='tddown'" onMouseUp="this.className='but'" onMouseOut="this.className='but01'">库存数量</td>
...
共 <%=total%> 条，当前第 <%=Mypage%>/<%=Maxpages%> 
            ...
            <%
			if search<>"" then
			url="cailiao_Store.asp?fenlei="&fenlei1&"&keyword="&keyword&"&search=search&"
			else
            url="cailiao_Store.asp?"
			end if				
...
<b><font color="#ffffff">库存搜索</font></b>
        ...
        <table width="800" border="0" cellpadding="0" cellspacing="0" bordercolor="#D4D0C8">
		  ...
		  <form name="search" method="post" action="cailiao_store.asp">
