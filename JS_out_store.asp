...
use_dep=trim(request.Form("use_dep"))
  ...
  response.write "<meta http-equiv=""refresh"" content=""0;url=JS_out_store.asp"">"
  response.end
else
...
<td >
			...
			<form action="JS_out_store.asp" method="post" name="selform" >
...
rs.Source="select* from JS_out_store where use_dep like '%"&keyword&"%' order by -id"
else
rs.Source="select* from JS_out_store  order by -id"
end if
...
<p align="right">共 <%=total%> 条，当前第 <%=Mypage%>/<%=Maxpages%> 
            ...
            <%
			if search<>"" then
			url="JS_out_store.asp?keyword="&keyword&"&search=search&"
			else
            url="JS_out_store.asp?"
			end if				
...
<b><font color="#ffffff">&nbsp;客 户 发 货 搜 索</font></b>
        ...
        <table width="100%" border="0" cellpadding="0" cellspacing="0" bordercolor="#D4D0C8">
		  ...
		  <form name="search" method="post" action="JS_out_store.asp">
