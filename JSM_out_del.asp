...
use_dep=trim(request.Form("use_dep"))
  ...
  response.write "<meta http-equiv=""refresh"" content=""0;url=JSM_out_del.asp"">"
  response.end
else
...
<td >
			...
			<form action="JSM_out_del.asp" method="post" name="selform" >
...
rs.Source="select* from chengping_out_store where use_dep like '%"&keyword&"%' order by -id"
else
rs.Source="select* from chengping_out_store order by -id"
end if
...
<p align="right">共 <%=total%> 条，当前第 <%=Mypage%>/<%=Maxpages%> 
            ...
            <%
			if search<>"" then
			url="JSM_out_del.asp?keyword="&keyword&"&search=search&"
			else
            url="JSM_out_del.asp?"
			end if				
...
<b><font color="#ffffff">&nbsp;客 户 发 货 搜 索</font></b>
        ...
        <table width="100%" border="0" cellpadding="0" cellspacing="0" bordercolor="#D4D0C8">
		  ...
		  <form name="search" method="post" action="JSM_out_del.asp">
