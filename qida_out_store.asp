...
response.write "<meta http-equiv=""refresh"" content=""0;url=qida_out_store.asp"">"
response.end
...
response.Write "<script language=javascript>alert('其它收入修改成功！');</script>"
response.write "<meta http-equiv=""refresh"" content=""0;url=qida_out_store.asp"">"
response.end
...
<%end if %>
        <table width="800" border="0" cellpadding="0" cellspacing="0" bordercolor="#0055E6">
		  ...
		  <form name="powersearch" method="post" action="qida_out_store.asp" onSubmit="return Juge(this)" >
...
<td height="18" align="center" class="but11" onMouseDown="this.className='tddown'" onMouseUp="this.className='but'" onMouseOut="this.className='but11'">
<input name="Loan_price" type="text" style="ime-mode:disabled"  size="10" value='<% if isedit then
...
rs.Source="select top 1000 * from chengping_out_store where pinming like '%"&keyword&"%' and use_dep like '%"&akeyword&"%'and guige=0 order by id desc "
else
rs.Source="select top 1000 * from chengping_out_store where guige=0 order by id desc "
end if
...
<td height="18" align="center"><a href="qida_out_store.Asp?id=<%=rs("ID")%>&action=edit"><%=rs("uptime")%></a></td>
<td height="18" align="center"><%=rs("fenlei")%><%if rs("fenlei")="" then%>&nbsp;<%end if%></td>
<td height="18" align="center">&nbsp;<a href="qida_out_store.Asp?id=<%=rs("ID")%>&action=edit"><%=rs("pinming")%></a></td>
...
<p align="right">共 <%=total%> 条，当前第 <%=Mypage%>/<%=Maxpages%> 
            ...
            <%
			if search<>"" then
			url="qida_out_store.asp?keyword="&keyword&"&akeyword="&akeyword&"&search=search&"
			else
            url="qida_out_store.asp?"
			end if				
...
<b><font color="#ffffff">销售搜索</font></b>
        ...
        <table width="100%" border="0" cellpadding="0" cellspacing="0" bordercolor="#D4D0C8">
		  ...
		  <form name="search" method="post" action="qida_out_store.asp">
