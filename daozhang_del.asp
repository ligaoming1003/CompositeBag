...
sub SaveModify  
  ...
  response.write "<meta http-equiv=""refresh"" content=""0;url=daozhang_del.asp"">"
  response.end
else
...
<td >
			...
			<form action="daozhang_del.asp" method="post" name="selform" >
...
<p align="right">共 <%=total%> 条，当前第 <%=Mypage%>/<%=Maxpages%> 
            ...
            <%
			if search<>"" then
			url="daozhang_del.asp?keyword="&keyword&"&search=search&"
			else
            url="daozhang_del.asp?"
			end if				
...
<b><font color="#ffffff">到账搜索</font></b>
        ...
        <table width="100%" border="0" cellpadding="0" cellspacing="0" bordercolor="#D4D0C8">
		  ...
		  <form name="search" method="post" action="daozhang_del.asp">
