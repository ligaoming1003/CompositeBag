...
if checkbox<>"" then
response.write "<meta http-equiv=""refresh"" content=""0;url=qida1_out_store.asp"">"
else
response.write "<meta http-equiv=""refresh"" content=""0;url=chengping_out_store.asp"">"
end if
response.end
...
if guige=0 or kuang=0 then
response.Write "<script language=javascript>alert('对不起，请在原来录入处修改！');</script>"
response.write "<meta http-equiv=""refresh"" content=""0;url=chengping_out_store.asp"">"
response.end
end if
...
rs.open sql,conn,1,3
if not rs.eof then
if num<0 and rs("number")+num<0 then
      response.Write "<script language=javascript>alert('对不起，库存不足，请调整您的出库量！');</script>"
      response.write "<meta http-equiv=""refresh"" content=""0;url=chengping_out_store.asp"">"
      response.end
else
...
sql2="select * from JS_out_store where pinming='"&pinming&"' and fenlei='"&fenlei&"' and comm="&comm&""
...
<TABLE width=800 align=center border="1" cellspacing="0" cellpadding="0" bordercolor="#0055E6">
  ...
  <TR> 
	...
	<%
	   ...
	   if isedit then
	   set rs=server.createobject("adodb.recordset")
		   rs.open "select top 3600 * from chengping_out_store where id=" & request("id"),conn,1,1
%>
...
<%end if %>
        <table width="800" border="0" cellpadding="0" cellspacing="0" bordercolor="#0055E6">
          <tr class="but"> 
			<td width="64" height="18" align="center" class="but" onMouseDown="this.className='tddown'" onMouseUp="this.className='but'" onMouseOut="this.className='but'">出库日期</td>
            <td width="80" height="18" align="center" class="but" onMouseDown="this.className='tddown'" onMouseUp="this.className='but'" onMouseOut="this.className='but'">类 别</td>
			<td width="656" height="18" align="center" class="but" onMouseDown="this.className='tddown'" onMouseUp="this.className='but'" onMouseOut="this.className='but'" colspan="3">
...
<td height="18" align="center" class="but11" onMouseDown="this.className='tddown'" onMouseUp="this.className='but'" onMouseOut="this.className='but11'" colspan="3">
		  ...
		  <tr> 
 <td height="18" align="center" class="but11" onMouseDown="this.className='tddown'" onMouseUp="this.className='but'" onMouseOut="this.className='but11'">
 <input name="Loan_price" type="text" style="ime-mode:disabled"  size="5" value='<% if isedit then
 ...
 <td height="18" align="center" class="but11" onMouseDown="this.className='tddown'" onMouseUp="this.className='but'" onMouseOut="this.className='but11'">
<input name="Loan_num" type="text" style="ime-mode:disabled"  size="8" value='<% if isedit then
...
<td  width="56" height="18" align="center" class="but11" onMouseDown="this.className='tddown'" onMouseUp="this.className='but'" onMouseOut="this.className='but11'">
<input type="checkbox" name="checkbox" value='yess'></td></tr>
			<tr>
...
<p align="left">　　　　　　　　　　　　&nbsp;&nbsp; 客户名　　『产品名』　　规　格　 单价</td>
</tr>
<tr class="but"> 
<td width="100%" height="25" align="center" class="but01" onMouseDown="this.className='tddown'" onMouseUp="this.className='but'" onMouseOut="this.className='but01'">
...
<%
		   ...
		   rs4.open "select top 1000 * from dindang order by id desc ",conn,1,1 
		   if not rs4.eof then%>
		   <option >请选择名称</option>
		   <%
		   do while not rs4.eof
			%>
			<option ><%=rs4("kehu")%> 『<%=rs4("pinming")%>』  <%=rs4("chang")%>*<%=rs4("kuang")%>&nbsp;&nbsp;&nbsp;<%if rs4("diangjia")<>"" then%>【<%=FormatNumber(rs4("diangjia"),3,-1) %>】<%else%>&nbsp;<%end if%></option>
...
<b><font color="#ffffff">产 品 销 售 列 表</font></b>
        ...
        <table width="100%" border="1" cellpadding="0" cellspacing="1" bordercolor="#EFEADD">
          <tr class="but"> 
			<td width="8%" height="18" align="center" class="but01" onMouseDown="this.className='tddown'" onMouseUp="this.className='but'" onMouseOut="this.className='but01'">出库日期</td>
			...
			<td width="37%" height="18" align="center" class="but01" onMouseDown="this.className='tddown'" onMouseUp="this.className='but'" onMouseOut="this.className='but01'">品 名</td>
			...
			<td width="8%" height="18" align="center" class="but01" onMouseDown="this.className='tddown'" onMouseUp="this.className='but'" onMouseOut="this.className='but01'">数 量</td>
			...
			<td width="7%" height="18" align="center" class="but01" onMouseDown="this.className='tddown'" onMouseUp="this.className='but'" onMouseOut="this.className='but01'">单价</td>
			<td width="9%" height="18" align="center" class="but01" onMouseDown="this.className='tddown'" onMouseUp="this.className='but'" onMouseOut="this.className='but01'">金额</td>
			<td width="8%" height="18" align="center" class="but01" onMouseDown="this.className='tddown'" onMouseUp="this.className='but'" onMouseOut="this.className='but01'">客户名称</td>
...
rs.Source="select top 3600 * from chengping_out_store where pinming like '%"&keyword&"%' and use_dep like '%"&akeyword&"%' and loan_num<>0 order by id desc"
else
rs.Source="select top 3600 * from chengping_out_store where loan_num<>0 order by id desc"
end if
...
<td height="18" align="center"><a href="chengping_out_store.Asp?id=<%=rs("ID")%>&action=edit"><%=rs("pinming")%><%if rs("content")<>"" then%>(<font color="#FF0000"><%=rs("content")%></font>)<%end if%></a></td>
<td height="18"  align="center"><%=rs("guige")%>*<%=rs("kuang")%></td>
<td height="18" align="center"><%if rs("fenlei")="卷材" then%><%=Formatnumber(rs("Loan_num"),2,-1,0,0)%><%else%> <%=rs("Loan_num")%><%end if%></td>
<td height="18" align="center"><%=rs("Unit")%><%if rs("Unit")="" then%>&nbsp;<%end if%></td>
<td height="18" align="center"><%=Formatnumber(rs("Loan_price"),4,-1,0,0)%><%if rs("Loan_price")="0" then%>&nbsp;<%end if%></td>
<td height="18" align="center"><%=Formatnumber(rs("Loan_Amount"),2,-1,0,0)%><%if rs("Loan_Amount")="0" then%>&nbsp;<%end if%></td>
