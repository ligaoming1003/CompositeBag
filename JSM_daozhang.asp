...
sub SaveModify  
  ...
  response.write "<meta http-equiv=""refresh"" content=""0;url=JSM_daozhang.asp"">"
  response.end
else
        conn.execute("delete from JS_daozhang where id in ("&Request.Form("selBigClass")&")")
...
<td >
			...
			<form action="JSM_daozhang.asp" method="post" name="selform" >
...
rs.Source="select* from JS_daozhang where kehu like '%"&keyword&"%' order by -id"
else
rs.Source="select* from JS_daozhang  order by -id"
end if
...
<p align="right">共 <%=total%> 条，当前第 <%=Mypage%>/<%=Maxpages%> 
            ...
            <%
			if search<>"" then
			url="JSM_daozhang.asp?keyword="&keyword&"&search=search&"
			else
            url="JSM_daozhang.asp?"
			end if				
