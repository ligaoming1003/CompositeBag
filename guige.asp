...
sql="select * from guige where guige="& request.Form("guige")&" "
rs.open sql,conn,1,3
if not rs.eof then
response.Write "<script language=javascript>alert('对不起，你现在添加的规格已经存在！');</script>"
response.write "<meta http-equiv=""refresh"" content=""0;url=guige.asp"">"
response.end
else
rs.addnew
...
sub SaveModify   
set rs=server.createobject("adodb.recordset") 
sql="select * from guige where id="&request.Form("id")
rs.open sql,conn,1,3
rs("guige") = request.Form("guige")
...
response.end
  ...
  sub delCate()
        conn.execute("delete from guige where id in ("&Request.Form("selBigClass")&")")
...
<TABLE width=600 align=center border="1" cellspacing="0" cellpadding="0" bordercolor="#0055E6">
  ...
  <TR> 
	...
	<%
	    set rs=server.createobject("adodb.recordset")
	   if isedit then
		   rs.open "select * from guige where id=" & request("id"),conn,1,1
%>

    <table width="600" border="0" cellpadding="0" cellspacing="0" >
          <tr >  
<td height=26 background="images/background.gif">
<b><font color="#ffffff">编 辑 宽 度 信 息</font></b>
...
%>
    <table width="600" border="0" cellpadding="0" cellspacing="0" >
          <tr >  
<td height=26 background="images/background.gif">
<b><font color="#ffffff">添 加 宽 度 信 息</font></b>
...
<%end if %>
       <table width="600" border="0" cellpadding="0" cellspacing="0" bordercolor="#0055E6">
		  ...
		  <form name="powersearch" method="post" action="guige.asp" onSubmit="return Juge(this)" >
...
<input name="guige" type="text" style="ime-mode:disabled"  size="25" value='<% if isedit then
...
<td >
			...
			<form action="guige.asp" method="post" name="selform" >
...
if search<>"" then
fenlei1=request("guige")
keyword=request("keyword")
rs.Source="select* from guige where guige like '%"&fenlei1&"%' order by id desc"
else
rs.Source="select* from guige order by id desc"
end if
...
<td height="18" align="center">&nbsp;&nbsp;<a href="guige.asp?id=<%=rs("ID")%>&action=edit"><%=rs("guige")%></a></td>
...
共 <%=total%> 条，当前第 <%=Mypage%>/<%=Maxpages%> 
            ...
            <%
			if search<>"" then
			url="guige.asp?guige="&fenlei1&"&keyword="&keyword&"&search=search&"
			else
            url="guige.asp?"
			end if				
...
<b><font color="#ffffff">&nbsp;宽 度 信 息 搜 索</font></b>
        ...
        <table width="600" border="0" cellpadding="0" cellspacing="0" bordercolor="#D4D0C8">
		  ...
		  <form name="search" method="post" action="guige.asp">
