...
sub delCate()
        conn.execute("delete from mxfuhe")
        conn.execute("delete from fuhe_in")
        conn.execute("delete from fuhe")
		response.Write "<script language=javascript>alert('已删除成功！');</script>"
        response.write "<meta http-equiv=""refresh"" content=""0;url=fuhe_del.asp"">"
        response.end
...
<td >
			...
			<form action="fuhe_del.asp" method="post" name="selform" >
...
共 <%=total%> 条，当前第 <%=Mypage%>/<%=Maxpages%> 
            ...
            <%
			if search<>"" then
			url="fuhe_del.asp?keyword="&keyword&"&search=search&"
			else
            url="fuhe_del.asp?"
			end if				
...
<b><font color="#ffffff">出库搜索</font></b>
        ...
        <table width="90%" border="0" cellpadding="0" cellspacing="0" bordercolor="#D4D0C8">
		  ...
		  <form name="search" method="post" action="fuhe_del.asp">
