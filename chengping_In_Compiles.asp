...
<table width="800" border="0" cellpadding="0" cellspacing="0">
	<form name="powersearch" method="post" action="chengping_In_Compiles.asp?action=search" >
  ...
  <select name='pinming'  style='width:130'>
		   ...
		   rs2.open "select distinct(pinming) from chengping_in_store order by pinming",conn,2,2
		   if not rs2.eof then
...
<td><select name="guige"  style='width:130'>
		   ...
		   rs3.open "select distinct(guige) from chengping_in_store order by guige",conn,2,2
		   if not rs3.eof then
...
<table width="800" border="1" cellpadding="0" cellspacing="1" bordercolor="#EFEADD">
          <tr class="but"> 
                        ...
                        <td width="59%" height="18" align="center" class="but" onMouseDown="this.className='tddown'" onMouseUp="this.className='but'" onMouseOut="this.className='but'">品 名</td>
                        <td width="8%" height="18" align="center" class="but" onMouseDown="this.className='tddown'" onMouseUp="this.className='but'" onMouseOut="this.className='but'">规 格</td>
...
<td height="18"align="center"><a href="chengping_In_Store.Asp?id=<%=rs("ID")%>&action=edit"><%=rs("uptime")%></a></td>
<td height="18" align="center"><font color="#CC3300"><%=rs("fenlei")%></font><%if rs("fenlei")="" then%>&nbsp;<%end if%></td>
<td height="18" align="center"><a href="chengping_In_Store.Asp?id=<%=rs("ID")%>&action=edit"><font color="#000099"><%=rs("pinming")%></font></a></td>
<td height="18" align="center"><%=rs("guige")%>*<%=rs("kuang")%></td>
<td height="18" align="center"><%=Formatnumber(rs("use_num"),2,-1,0,0)%></td>
<td height="18" align="center"><font color="#009900"><%=rs("Unit")%></font><%if rs("Unit")="" then%>&nbsp;<%end if%></td>
...
<%=use_num%></font></td></tr>
		...
		<br>
				<table cellpadding="0">
				<form name="search" method="post" action="chengping_In_Detail.asp" target="_blank" >
				  <tr><td>
				  ...
				  <input name="caigouyuan" type="hidden" value="<%=caigouyuan%>">
